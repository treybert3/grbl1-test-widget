     			...
     			switch(key){
     				case 'status':
						...
						if(this.work_mode===this.report_mode){
							this.publishAxisStatus({	"posx":parseFloat(this.last_work.x),
														"posy":parseFloat(this.last_work.y),
														"posz":parseFloat(this.last_work.z)
													});
                        } else if(this.work_mode===1 && this.report_mode===0){ //work is inch, reporting in mm
                            this.publishAxisStatus({	"posx":this.toInch(parseFloat(this.last_work.x)),
                            							"posy":this.toInch(parseFloat(this.last_work.y)),
                            							"posz":this.toInch(parseFloat(this.last_work.z))
                            						});
                        } else if(this.work_mode===0 && this.report_mode===1){ //work is mm, reporting in inch
                            this.publishAxisStatus({	"posx":this.toMM(parseFloat(this.last_work.x)),
                            							"posy":this.toMM(parseFloat(this.last_work.y)),
                            							"posz":this.toMM(parseFloat(this.last_work.z))
                            						});
                        }
     				break;
     				case 'gCodeState':
     				...
     				break;
     				case 'setting':
	                    this.config[parseInt(result[1],10)] = [parseFloat(result[2]), configStrings[result[1]]]; //save config value and description
     				break;
     				case 'message':
     				...
     				case 'hashQuery':
     					if(result[0] == 'PRB'){
							...
							if(this.work_mode===this.report_mode){
		                        ...
		                        chilipeppr.publish("/com-chilipeppr-interface-cnccontroller/proberesponse", {"x":this.toInch(parseFloat(coords[0]) - this.offsets.x),"y":this.toInch(parseFloat(coords[1]) - this.offsets.y),"z":this.toInch(parseFloat(coords[2]) - this.offsets.z), status: probeSuccess});
                    		}else if(this.work_mode===0 && this.report_mode===1){ //work is mm, reporting in inches
		                        chilipeppr.publish("/com-chilipeppr-interface-cnccontroller/proberesponse", {"x":this.toMM(parseFloat(coords[0]) - this.offsets.x),"y":this.toMM(parseFloat(coords[1]) - this.offsets.y),"z":this.toMM(parseFloat(coords[2]) - this.offsets.z), status: probeSuccess});
                			}                    
                        }
     				break;
     				case 'version':
